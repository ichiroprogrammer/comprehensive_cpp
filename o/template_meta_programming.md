...
c++20から導入された[コンセプト](cpp_standard.md#SS_19_10_3)によりジェネリックプログラミングは、
開発容易性、可読性、保守性が大きく向上しため、この章のコード例には、
[コンセプト](cpp_standard.md#SS_19_10_3)(`concept`, `requires`)を多用した。
...
[パラメータパック](cpp_standard.md#SS_19_10_4)を使って実装するべきである。

### パラメータパックを使用したテクニック <a id="SS_13_1_3"></a>
[パラメータパック](cpp_standard.md#SS_19_10_4)を使用するには独特なテクニックが必要となるため、まずは単純な例から説明する。
...
なお、上記コードで使用した[std::is_same](cpp_standard.md#SS_19_13_2_4)は、
...
Logging::Logger::set_innerでのApp::operator<<の[name lookup](cpp_standard.md#SS_19_11_2)について考えてみることにする。
...
その認識とは、「テンプレート内の識別子の[name lookup](cpp_standard.md#SS_19_11_2)は、
それがインスタンス化される時に行われる」というものであり、これにより
「 Logging::Loggerのname lookupは単体テスト内で行われる。
それはApp::operator<<宣言後であるためコンパイルできる」と考えることができるが、
[two phase name lookup](cpp_standard.md#SS_19_11_3)で行われるプロセスと反するため誤りである。
...
[ADL](cpp_standard.md#SS_19_11_5)(実引数依存探索)が働く。
また、Logging::Logger::set_inner(x)はテンプレートであるため、[two phase name lookup](cpp_standard.md#SS_19_11_3)
が使用される。
...
instsの[関連名前空間](cpp_standard.md#SS_19_11_6)もstdであり、Appではない。
その結果App::operator<<は発見できず、このようなエラーになった。

LOGGERからApp::operator<<を使う場合の単体テストは下記のようになるが、
[ADL](cpp_standard.md#SS_19_11_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdのみであり、
前記単体テスト同様にコンパイルできない。
...
「[ADL](cpp_standard.md#SS_19_11_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdである」
...
上記コードは[two phase name lookup](cpp_standard.md#SS_19_11_3)等の効果により、想定通りコンパイルできるが、
...
このドキュメントで使用している[g++](cpp_jargon.md#SS_20_3_1)ではこのコードはコンパイルでき、
動作も問題ないように思われるが、[clang++](cpp_jargon.md#SS_20_3_2)では以下のようなエラーが発生し、コンパイルできない。
...
この理由は「[two phase name lookup](cpp_standard.md#SS_19_11_3)」の後半で詳しく解説したので、ここでは繰り返さないが、
...
* operator<<(std::ostream& os, App::Ints_t const& ints)
  ...
  (「[割れ窓理論](term_explanation.md#SS_21_7_1)」参照)になってしまうかもしれない)。
...
これまで述べてきたようにApp::Inst_tの[関連名前空間](cpp_standard.md#SS_19_11_6)がAppではなく、stdになってしまうからである。

これを回避するためにはその原因を取り払えばよく、
つまり、App::Inst_tの[関連名前空間](cpp_standard.md#SS_19_11_6)がAppになるようにすればよい。
...
* using宣言によりstd::vectorのすべてのコンストラクタをApp::Ints_tに導入
  (「[継承コンストラクタ](cpp_standard.md#SS_19_5_4)」参照)

としているため、エイリアスで宣言されたInts_tと等価である。
C++03では、[継承コンストラクタ](cpp_standard.md#SS_19_5_4)が使えなかったため、
上記のような構造体を定義するためには、
...
コードクローンや、[車輪の再発明](term_explanation.md#SS_21_7_2)による開発効率の低下につながることがある。
...
これを初期化するためには初期化子リスト
(「[初期化子リストコンストラクタ](cpp_standard.md#SS_19_5_3)」、
「[一様初期化](cpp_standard.md#SS_19_5_2)」参照)を用いるのが良い。
...
* [SFINAE](cpp_standard.md#SS_19_10_1)
...
本節では、[type_traits](cpp_standard.md#SS_19_13_2)の[メタ関数](cpp_standard.md#SS_19_10_2)の実装等で広く使われいる下記のようなテクニックを

- [std::is_void](cpp_standard.md#SS_19_13_2_7)と同等のメタ関数を[is_void_xxxの実装](template_meta_programming.md#SS_13_3_1)
- [std::is_same](cpp_standard.md#SS_19_13_2_4)と同等のメタ関数を[is_same_xxxの実装](template_meta_programming.md#SS_13_3_2)
...
[SFINAE](cpp_standard.md#SS_19_10_1)を使用した関数テンプレートis_void_sfinae_fの実装は以下のようになる。
...
であるため、Tがvoidの時のみ[name lookup](cpp_standard.md#SS_19_11_2)の対象になる。
...
であるため、Tが非voidの時のみ[name lookup](cpp_standard.md#SS_19_11_2)の対象になる。
...
[SFINAE](cpp_standard.md#SS_19_10_1)を使用したクラステンプレートis_void_sfinae_sの実装は以下のようになる。
...
is_void_sfinae_sの特殊化が[name lookup](cpp_standard.md#SS_19_11_2)の対象の中に見つからなかった場合、
これが使われる。
...
[std::enable_if](cpp_standard.md#SS_19_13_2_5)による[SFINAE](cpp_standard.md#SS_19_10_1)とクラステンプレートの特殊化を使用した
...
[std::conditional](cpp_standard.md#SS_19_13_2_6)と関数テンプレートの特殊化を使用したis_void_cond_sの実装は以下のようになる。
...
この例では、[SFINAE](cpp_standard.md#SS_19_10_1)もクラステンプレートの特殊化も使用していないが、
...
|[same_as](template_meta_programming.md#SS_13_3_2_6)                |[コンセプト](cpp_standard.md#SS_19_10_3)よるis_same_sfinae_sと同一の機能      |
...
[SFINAE](cpp_standard.md#SS_19_10_1)による[is_same_sfinae_s](template_meta_programming.md#SS_13_3_2_5)の難解なコードを[コンセプト](cpp_standard.md#SS_19_10_3)
...
* 第1パラメータが第2パラメータ以降で指定された型のどれかと一致する
  SameAsSomeOfという名前の[コンセプト](cpp_standard.md#SS_19_10_3)を[畳み込み式](cpp_standard.md#SS_19_10_5)を使用し定義する
...
OneOfの実装にはシンプルに記述するための[畳み込み式](cpp_standard.md#SS_19_10_5)を使用した。
...
上記を抜粋した下記のコードは「縮小型変換を発生さる{}による初期化は[ill-formed](cpp_standard.md#SS_19_14_1)になる」
ことをSFINAEに利用している。
...
|[Beginable/Endable](template_meta_programming.md#SS_13_3_4_8)              |[コンセプト](cpp_standard.md#SS_19_10_3)を使用したexists_begin/exsits_endを単純化した例   |
|[IsRange](template_meta_programming.md#SS_13_3_4_6)                        |exists_begin/exsits_endを使し、範囲forのオペランドになれるか?の判断|
|[Ranged](template_meta_programming.md#SS_13_3_4_9)                         |機能はIsRangeと同一だが、[コンセプト](cpp_standard.md#SS_19_10_3)を使用しSFINAEの回避     |
...
以下に示すように、C++20から導入された[コンセプト](cpp_standard.md#SS_19_10_3)はこのような問題の軽減につながる。
...
* [ill-formed](cpp_standard.md#SS_19_14_1)ならばill-formedになる
* [well-formed](cpp_standard.md#SS_19_14_2)ならvoidを生成する
...
の戻り型が配列型の[rvalue](cpp_standard.md#SS_19_6_1_2)である"int (&&) [3]"となり、
...
そのオブジェクトが[lvalue](cpp_standard.md#SS_19_6_1_1)(この例ではint (&)[3])であれば、
...
* [SFINAE](cpp_standard.md#SS_19_10_1)を利用した[ExistsPutTo](template_meta_programming.md#SS_13_3_4_13)は複雑で醜いため、リファクタリングする。
...
このような記述が必要になる。
一方で[コンセプト](cpp_standard.md#SS_19_10_3)を使用したC++20スタイルのSFINAEの可読性の高さを実感できただろう。
...
値を型に変換する[std::integral_constant](cpp_standard.md#SS_19_13_2_1)を使用し、この問題を解決できる。
...
ここでも、すでにみてきた[コンセプト](cpp_standard.md#SS_19_10_3)による制約によりこの問題に対処する。
...
実際、現在使用中の[g++](cpp_jargon.md#SS_20_3_1)ではこのコードはコンパイルできず、
...
[forwardingリファレンス](cpp_standard.md#SS_19_7_3)を使って下記のように書き直した。
...
この原因は、
「関数が受け取った[rvalue](cpp_standard.md#SS_19_6_1_2)リファレンスは、
その関数から別の関数に受け渡される時に[lvalue](cpp_standard.md#SS_19_6_1_1)リファレンスとして扱われる」からである。
...
[畳み込み式](cpp_standard.md#SS_19_10_5)を使用した下記の実装の方がより明確である。
...
(T const&&はconstな[rvalue](cpp_standard.md#SS_19_6_1_2)リファレンスである)、
forwardingリファレンスが[lvalue](cpp_standard.md#SS_19_6_1_1)リファレンスであった場合は、
...
なお、forwardingリファレンスは、[リファレンスcollapsing](cpp_standard.md#SS_19_7_6)の一機能としても理解できる。
...
この記法は[ジェネリックラムダ](cpp_standard.md#SS_19_10_6)と呼ばれる。
...
[ADL](cpp_standard.md#SS_19_11_5)(実引数依存探索)により、Appもis_equalの[name lookup](cpp_standard.md#SS_19_11_2)の対象になるからである。
...
* [std::enable_if](cpp_standard.md#SS_19_13_2_5)や[コンセプト](cpp_standard.md#SS_19_10_3)等を使用してテンプレートに適用できる型を制約する
...
([two phase name lookup](cpp_standard.md#SS_19_11_3)の一回目のname lookupでfがバインドされるため)。
...
[ADL](cpp_standard.md#SS_19_11_5)や[two phase name lookup](cpp_standard.md#SS_19_11_3)が絡む場合ならなおさらである。

こういう意図しない[name lookup](cpp_standard.md#SS_19_11_2)に備えるためには、
...
XY型オブジェクトを引数にした関数呼び出しによる[関連名前空間](cpp_standard.md#SS_19_11_6)は、
...
等によるコンパイルエラーとの戦いである。
また、これをクリアしても[two phase name lookup](cpp_standard.md#SS_19_11_3)や[ADL](cpp_standard.md#SS_19_11_5)が次の関門になる。
...
残念なことにこの関数は非標準であるため、
それを使っているNstd::Inner\_::demangleは[g++](cpp_jargon.md#SS_20_3_1)/[clang++](cpp_jargon.md#SS_20_3_2)
でなければコンパイルできないだろう。
...
[暗黙の型変換](term_explanation.md#SS_21_4_1)を利用した文字列リテラルからStaticStringオブジェクトへの変換は、
...
* [name lookup](cpp_standard.md#SS_19_11_2)には複雑なルールが適用されるため、非直感的なバインドが行われる場合がある。
  従って、テンプレートライブラリの開発には単体テストは必須である。

* 使用しているコンパイラが[two phase name lookup](cpp_standard.md#SS_19_11_3)をサポートしているか否かに気を付ける。
...
* 可変長引数を持つ関数の実装には[パラメータパック](cpp_standard.md#SS_19_10_4)を使う。

* 処理速度や関数のリターンの型に影響する場合があるため、
  パラメータパックの処理の順番に気を付ける(「[前から演算するパラメータパック](template_meta_programming.md#SS_13_1_3_2)」参照)。

* [ADL](cpp_standard.md#SS_19_11_5)を利用しない場合、テンプレートで使う識別子は名前空間名やthis->等で修飾する
...
* [forwardingリファレンス](cpp_standard.md#SS_19_7_3)の実際の型がlvalueリファレンスであるならば、
  constなlvalueリファレンスとして扱う
  (「[実引数/仮引数](programming_convention.md#SS_3_3_3_5)」参照)
  。

* ユニバーサルリファレンス引数を他の関数に渡すのであれば、std::forwardを使う
  (「[forwardingリファレンス](cpp_standard.md#SS_19_7_3)」、「[forwardingリファレンスとstd::forward](template_meta_programming.md#SS_13_7_1)」参照)。

* 関数テンプレートとその特殊化はソースコード上なるべく近い位置で定義する
  (「[two phase name lookup](cpp_standard.md#SS_19_11_3)」参照)。

* [two phase name lookup](cpp_standard.md#SS_19_11_3)により意図しない副作用が発生する可能性があるため、
...
* 関数テンプレートのオーバーロードと特殊化の[name lookup](cpp_standard.md#SS_19_11_2)の優先度に気を付ける。
...
* 意図しないテンプレートパラメータによるインスタンス化の防止や、
  ...
  static_assertによるテンプレートパラメータの制約よりも、[コンセプト](cpp_standard.md#SS_19_10_3)による制約を優先する。
