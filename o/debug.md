...
&emsp;&emsp; [デバッガの使用](debug.md#SS_14_1)  
&emsp;&emsp; [gitの使用](debug.md#SS_14_2)  
&emsp;&emsp; [TATの短縮](debug.md#SS_14_3)  
&emsp;&emsp;&emsp; [ビルド時間の短縮](debug.md#SS_14_3_1)  
&emsp;&emsp;&emsp; [依存関係の整理](debug.md#SS_14_3_2)  

&emsp;&emsp; [自動単体テストの実施](debug.md#SS_14_4)  
&emsp;&emsp; [自動統合テストの実施](debug.md#SS_14_5)  
&emsp;&emsp; [事後的デバッグ](debug.md#SS_14_6)  
&emsp;&emsp;&emsp; [ログの取得](debug.md#SS_14_6_1)  
&emsp;&emsp;&emsp; [モニター](debug.md#SS_14_6_2)  

&emsp;&emsp; [潜在的バグの削減](debug.md#SS_14_7)  
&emsp;&emsp; [難しいバグの対処](debug.md#SS_14_8)  
&emsp;&emsp; [助成の依頼](debug.md#SS_14_9)  
&emsp;&emsp;&emsp; [現象1 - 不思議なハングアップ](debug.md#SS_14_9_1)  
&emsp;&emsp;&emsp; [現象2 - グローバル変数の破壊](debug.md#SS_14_9_2)  
&emsp;&emsp;&emsp; [現象3 - プロセスのスローダウン](debug.md#SS_14_9_3)  
&emsp;&emsp;&emsp; [現象4 - バックトレースが見れない](debug.md#SS_14_9_4)  
&emsp;&emsp;&emsp; [現象5 - newしたオブジェクトの破壊](debug.md#SS_14_9_5)  
&emsp;&emsp;&emsp; [現象6 - STLのバグ?](debug.md#SS_14_9_6)  
&emsp;&emsp;&emsp; [現象7 - 解放後のrvalueへのアクセス](debug.md#SS_14_9_7)  

&emsp;&emsp; [まとめ](debug.md#SS_14_10)  
...
また、「[自動単体テストの実施](debug.md#SS_14_4)」に従えば、組み込みソフト開発においても、
...
本ドキュメントでは、「[開発プロセスとインフラ](process_and_infra.md#SS_10)」で述べたように、単体テスト、
統合テストを自動化することを推奨している。

本ドキュメントで使用するディレクトリやファイル間の依存関係を解析するためのソフトウェア
[deps](deps.md#SS_16)の開発においても、当然これを実践している。
...
のような状態に陥ったとする([CI(継続的インテグレーション)](process_and_infra.md#SS_10_2_5)を導入しているソフトウェア開発では、
このような状態は珍しいことでない)。
...
## TATの短縮 <a id="SS_14_3"></a>
TATとはターンアラウンドタイムの略語であり、一度の試行に要する時間を指す。
デバッグのTATが短ければ短いほどデバッグ効率が良くなることは明らかだろう
(これとは対をなす概念が「[事後的デバッグ](debug.md#SS_14_6)」である)。

再現性の悪いバグには、様々なテストパターンを試す、環境を変えてみる、
等の運任せの手法も時には必要になるが、以下のような戦略的アプローチが最も有効である。

* printfデバッグを使わない(「[デバッガの使用](debug.md#SS_14_1)」参照)
* [ビルド時間の短縮](debug.md#SS_14_3_1)
* [自動単体テストの実施](debug.md#SS_14_4)(ターゲットハードウェアへのプログラム転送が不要なため、
  組み込みソフト開発には特に有効である)

### ビルド時間の短縮 <a id="SS_14_3_1"></a>
...
* 差分ビルドが有効に働くように、[依存関係の整理](debug.md#SS_14_3_2)を行う
* 不要なインクルードを行わない
  (静的解析ツールによっては、不要なインクルードを指摘してくれる)

ビルドツールが並列コンパイルをサポートしているのであれば
(「[コンパイラ](dev_tools.md#SS_15_1)」で述べたしたメーカのIDEは並列コンパイルができなかった)、
上記したような高スペックのビルドマシンを導入することで、
工数負担なしにコンパイル時間の改善ができる。
多少費用は増えるが、人件費とは比較にならないほど安価である(「[開発ツール](dev_tools.md#SS_15)」参照)。

各プログラマがフルビルドを一日に何度も行うことはないので、
実践的には差分ビルドが有効になるように依存関係を見直すのが良い。
ただし、これには「[依存関係の整理](debug.md#SS_14_3_2)」で説明したような努力が必要である。

### 依存関係の整理 <a id="SS_14_3_2"></a>
...
* 単体テストが容易になる(「[自動単体テスト](process_and_infra.md#SS_10_2_1)」参照)
* 修正後の影響範囲を明確にしやすい
* ライブラリの転用がしやすい
...
* 静的解析ツール等を使った依存関係の監視(「[deps](deps.md#SS_16)」参照)
* ビルドツールの使い方の工夫(「[makeによる依存関係の維持](deps.md#SS_16_3)」参照)

等が必要になる。

## 自動単体テストの実施 <a id="SS_14_4"></a>
[自動単体テスト](process_and_infra.md#SS_10_2_1)は、

* デバッグのTATを短縮できる(「[TATの短縮](debug.md#SS_14_3)」参照)
...
## 自動統合テストの実施 <a id="SS_14_5"></a>
[自動統合テスト](process_and_infra.md#SS_10_2_3)は、

* ほぼ工数をかけることなく、回帰テストを実施できるため、
  共有リポジトリ内のソースコードをクリーンに保てる
* 長時間操作が必要なテスト(メモリーリークや、発生率の悪いタイミング問題等)を、
  人手をかけずに実施できる
* git bisect(「[gitの使用](debug.md#SS_14_2)」参照)と組み合わせれば、
  デグレードが起こった共有リポジトリのリビジョンを特定できる

等の効果を期待できるため、現在のソフトウェア開発において欠かすことができない手法である。

[deps](deps.md#SS_16)に関しても、以下のシェルスクリプトにより自動統合テストを実現している。
...
[deps](deps.md#SS_16)の開発にもログは有効であり、

* ログ取得用のクラスと関数テンプレート
  (「[ログ取得ライブラリの開発](template_meta_programming.md#SS_12_1)」、「[ログ取得ライブラリの開発2](template_meta_programming.md#SS_12_6)」参照)
...
[モニター](debug.md#SS_14_6_2)の機能によりそれを吸い上げる必要がある(ファイルが使える場合にもこれは同様である)。

### モニター <a id="SS_14_6_2"></a>
...
* デバッガの基本機能
    ...
    * リアルタイムOSのリソースの表示
    * スレッドの状態の表示や[スタック消費量の計測](debug.md#SS_14_6_2_2)
    * [メモリ保護機能](debug.md#SS_14_6_2_1)のON/OFF
    * グローバルなnew/deleteの使用状況(「[グローバルnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_13_4_1)」参照)
...
[CI(継続的インテグレーション)](process_and_infra.md#SS_10_2_5)と組み合わせれば、
効率的に運用できる(「[自動統合テストの実施](debug.md#SS_14_5)」参照)。

なお、[deps](deps.md#SS_16)に関しても、以下のようにすることで単体テスト、
...
ハングアップについては、[ログの取得](debug.md#SS_14_6_1)、[デバッガの使用](debug.md#SS_14_1)
が有効なデバッグ方法となることが多い(もちろん組み込みの場合は、[モニター](debug.md#SS_14_6_2)が必要である)。
また、バージョンが特定できるのであればgitのdiff/bisect等(「[デバッガの使用](debug.md#SS_14_1)」参照)が役に立つ。

リソースの枯渇については、
scan-buildやcoverity等の静的解析ツール(「[潜在的バグの削減](debug.md#SS_14_7)」参照)を使うことで、
ピンポイントでリーク箇所を特定できることがあるため、
まずはこれらを使ってみることを推奨する。
これで発見できない場合、動的解析ツールの使用を検討するべきだろう。

恒久的には、 「[RAII(scoped guard)](cpp_idioms.md#SS_20_1_2)」で紹介したようなリソースの自動解放のパターンを使用し、
そもそもこのような問題が発生しないように改善するべきだろう。

バグの性質上、再現/回帰テストには長時間のアプリケーション動作が必要になることがある。
テスターにひたすらクリックさせるようなテストは非効率であるだけでなく、
人道的にも問題になるかもしれないため、[自動統合テストの実施](debug.md#SS_14_5)も必須になる。
...
そのような場合は、上級なプログラマに助成(「[助成の依頼](debug.md#SS_14_9)」参照)を頼むこと以外、
建設的なデバッグ方法はない
(やみくもにソースコードを眺めたり、
ログやprintfを仕込んでも、よほどの幸運でもない限りバグは見つからない)。

## 助成の依頼 <a id="SS_14_9"></a>
...
「一緒にデバッグしてもらう」相手は、
「[難しいバグの対処](debug.md#SS_14_8)」で述べたような知識豊富なプログラマで無ければならない。
...
### 現象1 - 不思議なハングアップ <a id="SS_14_9_1"></a>
...
* カレントスレッドポインタは静的変数であるため、
  「[現象2 - グローバル変数の破壊](debug.md#SS_14_9_2)」のようなことがない限り、RTOSのバグである

「[現象2 - グローバル変数の破壊](debug.md#SS_14_9_2)」で書いたような方法により、
...
### 現象2 - グローバル変数の破壊 <a id="SS_14_9_2"></a>
...
参考のため、「[deps](deps.md#SS_16)」の実行形式バイナリにnmを適用した例を示す。
...
もし、「[デバッガの使用](debug.md#SS_14_1)」で説明したようなことが可能ならば、
...
### 現象3 - プロセスのスローダウン <a id="SS_14_9_3"></a>
...
### 現象4 - バックトレースが見れない <a id="SS_14_9_4"></a>
...
### 現象5 - newしたオブジェクトの破壊 <a id="SS_14_9_5"></a>
...
多くの場合、「[潜在的バグの削減](debug.md#SS_14_7)」で触れたようなコード解析で発見できるが、
...
### 現象7 - 解放後のrvalueへのアクセス <a id="SS_14_9_7"></a>
下記の関数gen_strは、
...
[rvalue](core_lang_spec.md#SS_18_7_1_2)を引数に取ることができる。
