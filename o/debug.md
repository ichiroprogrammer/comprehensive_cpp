...
[デバッガの使用](debug.md#SS_15_1)  
[gitの使用](debug.md#SS_15_2)  
[TATの短縮](debug.md#SS_15_3)  
&emsp;[ビルド時間の短縮](debug.md#SS_15_3_1)  
&emsp;[依存関係の整理](debug.md#SS_15_3_2)  

[自動単体テストの実施](debug.md#SS_15_4)  
[自動統合テストの実施](debug.md#SS_15_5)  
&emsp;[ログの取得](debug.md#SS_15_5_1)  

[組み込みソフト](debug.md#SS_15_6)  
&emsp;[モニター](debug.md#SS_15_6_1)  
&emsp;[メモリ保護](debug.md#SS_15_6_2)  
&emsp;[割り込みベクタ](debug.md#SS_15_6_3)  
&emsp;[assert(false)](debug.md#SS_15_6_4)  
&emsp;[スタック消費量の計測](debug.md#SS_15_6_5)  

[潜在的バグの削減](debug.md#SS_15_7)  
[難しいバグの対処](debug.md#SS_15_8)  
[助成の依頼](debug.md#SS_15_9)  
&emsp;[現象1 - 不思議なハングアップ](debug.md#SS_15_9_1)  
&emsp;[現象2 - グローバル変数の破壊](debug.md#SS_15_9_2)  
&emsp;[現象3 - プロセスのスローダウン](debug.md#SS_15_9_3)  
&emsp;[現象4 - バックトレースが見れない](debug.md#SS_15_9_4)  
&emsp;[現象5 - newしたオブジェクトの破壊](debug.md#SS_15_9_5)  
&emsp;[現象6 - STLのバグ?](debug.md#SS_15_9_6)  
&emsp;[現象7 - 解放後のrvalueへのアクセス](debug.md#SS_15_9_7)  

[まとめ](debug.md#SS_15_10)  
...
「[ソースコードファイルとディレクトリ](programming_convention.md#SS_3_7)」で述べたようにソースコードを[パッケージ](cpp_idioms.md#SS_21_9_2)分割し、
...
[モニター](debug.md#SS_15_6_1)の機能によりそれを吸い上げる必要がある(ファイルが使える場合にもこれは同様である)。

## 組み込みソフト <a id="SS_15_6"></a>
多くのバグは、組織の公式なテストやユーザによって発見される(ユーザには発見されたくないが)。
そのため、バグ発生時にデバッガが使えない状況は珍しくない。
ホスト上のソフトウェアであれば、メモリダンプを取得し、事後的にデバッガを利用できる。
しかし、このアプローチが常に効果的とは限らない。
組み込みソフトの場合は、メモリダンプの取得自体が容易ではない。

ここでは、そのような状況下で有効な手法について解説する。

### モニター <a id="SS_15_6_1"></a>
組み込みソフトの場合、バグ発生後にメモリダンプを取り、後からデバッガで調査することは難しい。
...
* [割り込みベクタ](debug.md#SS_15_6_3)/[assert(false)](debug.md#SS_15_6_4)状態から実行できる必要があるため、
  割り込みに依存しない方法で実装する必要がある。
  従って、uartドライバはIRQを利用するものとIRQを利用しないポーリングドライバを開発し、
  モニターからこれを切り替えて使えるようにしなければならない。
...
* デバッグ基本機能
    ...
    * リアルタイムOSのリソースの表示
    * スレッドの状態の表示や[スタック消費量の計測](debug.md#SS_15_6_5)
    * [メモリ保護](debug.md#SS_15_6_2)のON/OFF
    * 「[ログの取得](debug.md#SS_15_5_1)」のレベル変更やログ取得済ログの表示
...
ARMを使っているのであれば、[https://developer.arm.com/](https://developer.arm.com/)
等のネットドキュメントに十分情報が掲載されている。他CPUにも同様のドキュメントは提供されているはずである。

モニターの実装難度が高い、もしくは時間がない等の理由でこういった機能の実装をおろそかにすれば、
必ずと言っていいほど極度に解析困難なバグに悩まされることになる。

### メモリ保護 <a id="SS_15_6_2"></a>
組み込みソフトは、[フリースタンディング環境](cpp_idioms.md#SS_21_14_9)で動作させることが一般的である。
その場合、CPUのデフォルトの設定では、読み取り専用領域
(インストラクション、文字列リテラル、constなPOD等)に対しても書き込みできるため、
例えば、構造体ポインタ(struct_ptr)にnullptrが代入された状態で、
`struct_ptr->m = x;`が実行されば0アドレス付近のコードが破壊される。

多くのCPUでは、この領域には割り込みベクタが配置されているため、[CPU例外](cpp_idioms.md#SS_21_14_11)が発生する。
それを捕捉する機能が実装されていなければソフトは暴走する。CPU例外を捕捉する機能が実装されている場合でも、
その例外コードは「不正なインストラクションの実行」を示すことがほとんどであるため真の原因特定は難しい。

[メモリ保護機構](cpp_idioms.md#SS_21_14_10)を適切に設定することにより、上記のような暴走を回避し、
問題のインストラクションを実行したアドレスを特定することは容易である。
後は、ツールチェーンに付属するaddr2line、readelf、objdump、nmのようなバイナリ解析ツールを使い、
アドレスからソースコードを特定すればよい。

ただし、jtagデバッガを使用する場合、[メモリ保護機構](cpp_idioms.md#SS_21_14_10)の設定には注意が必要である。
jtagデバッガがブレークポイントを設定する方法は2つある。一つはハードウェアブレークであり、 
他方はソフトウェアブレークである。ソフトウェアブレークを設定する場合、 
デバッガは、
一時的に「ブレークを指定されたソースコードに対応するインストラクション」 に、
「ソフトウェアブレークするためのインストラクション」を上書きする。

メモリ保護をしている場合、jtagデバッガによるこの動作はCPU例外を発生させることがある。 これを避けるためには、
一時的に[メモリ保護](debug.md#SS_15_6_2)を無効化すればよい。
これを[モニター](debug.md#SS_15_6_1)から実行できるコマンドとして実装すると便利である。

### 割り込みベクタ <a id="SS_15_6_3"></a>
ほとんどの組み込みソフトではIRQを捕捉するための処理は実装されているが、
その一方で、[CPU例外](cpp_idioms.md#SS_21_14_11)を捕捉する処理が実装されていないことが多い。

CPU例外を捕捉する処理が未実装の場合、0除算やメモリ保護違反が発生すると、システムは予測不能な動作をする。
最悪の場合、割り込みベクタ領域が破壊され、以降の割り込み処理が正常に動作しなくなる。
これを防ぐには、すべてのCPU例外に対応するハンドラを実装し、
例外発生時のCPUレジスタ一式やスタックトレースを記録してモニターに制御を移す仕組みが必要である。

### assert(false) <a id="SS_15_6_4"></a>
組み込みソフトにおいてもassertが使えるようにすることは重要である。
assert(false)となった場合、モニターを呼び出し、デバッグできる状態に移行することが開発効率を向上させる。
標準ライブラリのassertは、条件が偽の場合にabort()を呼び出すが、
[フリースタンディング環境](cpp_idioms.md#SS_21_14_9)ではこれが適切に動作しないことが多い。
そのため、assertマクロを独自に実装し、失敗時にファイル名、行番号、スタックトレースを記録した後、
モニターに制御を渡すようにすることで、assert(false)の原因を効率よく調査できる。

### スタック消費量の計測 <a id="SS_15_6_5"></a>
...
ハングアップについては、[ログの取得](debug.md#SS_15_5_1)、[デバッガの使用](debug.md#SS_15_1)
が有効なデバッグ方法となることが多い(もちろん組み込みの場合は、[モニター](debug.md#SS_15_6_1)が必要である)。
...
恒久的には、 「[RAII(scoped guard)](cpp_idioms.md#SS_21_1_3)」で紹介したようなリソースの自動解放のパターンを使用し、
