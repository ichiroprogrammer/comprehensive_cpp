...
* 母音等を省略しない。
  下記のコード例のようにsignalの意味でsigを使用する場合は、その省語を[略語リスト](naming_practice.md#SS_6_1_1)リストに掲載する。
...
* 対称的概念には対称的な名前(startの対義語は stop、beginの対義語はend であるため、
  start/end の組み合わせは語源的に非対称となる。[言葉の対称性](naming_practice.md#SS_6_1_2)」参照)。
...
この際、[クラスのコメント](comment.md#SS_7_2_1)、[関数のコメント](comment.md#SS_7_2_2)のフォーマットを生成AIに伝えれば、
