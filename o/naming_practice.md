...
* ソフトウェア構成物の意味や責務、性質を容易に想起できる。
    * 一般に使用されている、もしくはプロジェクトで定義(「[略語リスト](naming_practice.md#SS_5_1_1)」参照)
      されている略語以外の略語は使用しない(特に母音の省略は悪習慣である)。
    * 対称的な概念には、対称的な名前を付ける(「[言葉の対称性](naming_practice.md#SS_5_1_2)」参照)。
...
5.1 [禁止事項](naming_practice.md#SS_5_1)  
&emsp;5.1.1 [略語リスト](naming_practice.md#SS_5_1_1)  
&emsp;5.1.2 [言葉の対称性](naming_practice.md#SS_5_1_2)  

5.2 [識別子の命名](naming_practice.md#SS_5_2)  
&emsp;5.2.1 [ファイル、ディレクトリの命名](naming_practice.md#SS_5_2_1)  
&emsp;5.2.2 [ファイル名](naming_practice.md#SS_5_2_2)  
&emsp;5.2.3 [ディレクトリ名](naming_practice.md#SS_5_2_3)  
&emsp;5.2.4 [型名](naming_practice.md#SS_5_2_4)  
&emsp;5.2.5 [定数名](naming_practice.md#SS_5_2_5)  
&emsp;5.2.6 [ローカル変数名(自動変数名、仮引数名、関数内static変数名)](naming_practice.md#SS_5_2_6)  
&emsp;5.2.7 [メンバ変数名](naming_practice.md#SS_5_2_7)  
&emsp;5.2.8 [メンバ関数名](naming_practice.md#SS_5_2_8)  
&emsp;5.2.9 [Accessorメンバ関数名](naming_practice.md#SS_5_2_9)  
&emsp;5.2.10 [関数名](naming_practice.md#SS_5_2_10)  
&emsp;5.2.11 [名前空間名](naming_practice.md#SS_5_2_11)  
&emsp;5.2.12 [テンプレート名](naming_practice.md#SS_5_2_12)  
&emsp;5.2.13 [テンプレートの仮引数名](naming_practice.md#SS_5_2_13)  
&emsp;5.2.14 [マクロ名](naming_practice.md#SS_5_2_14)  
&emsp;5.2.15 [その他の命名則](naming_practice.md#SS_5_2_15)  
...
## 5.1 禁止事項 <a id="SS_5_1"></a>

* 以下のような識別子はC++の予約語やそれと紛らわしいため、定義/宣言しない。
...
* 母音等を省略しない。
  下記のコード例のようにsignalの意味でsigを使用する場合は、その省語を[略語リスト](naming_practice.md#SS_5_1_1)リストに掲載する。
...
* 英文字の大小で識別子を区別しない。
  ただし、「[NVI(non virtual interface)](cpp_idioms.md#SS_17_1_8)」を使用するメンバ関数名は例外とする。
...
* 一般的でない略語を使わない。
  ...
  (「[略語リスト](naming_practice.md#SS_5_1_1)」参照)。
* 対称的概念には対称的な名前(startの対義語は stop、beginの対義語はend であるため、
  start/end の組み合わせは語源的に非対称となる。[言葉の対称性](naming_practice.md#SS_5_1_2)」参照)。

### 5.1.1 略語リスト <a id="SS_5_1_1"></a>
...
|:--------------------------|:----------------------------------------------------------------------------|
| cmd                       | command                                                                     |
| sig                       | signal(signalに対してのsigはPOSIXで広く使われる略語(sighandler_t等)         |          
| fd                        | file descriptor                                                             |
| lhs                       | left-hand side                                                              |
| num                       | number                                                                      |
| prev                      | previous                                                                    |
...
## 5.2 識別子の命名 <a id="SS_5_2"></a>
コードの可読性に大きな影響を与える識別子名は[パッケージ](cpp_idioms.md#SS_17_9_2)外部に公開されるクラス、関数、名前空間等の名称である。

クラス名、関数名、名前空間名に良い名前を与えるためにこの章が存在するのだが、
執筆中に生成AI(chatgpt、claude、gemini等)が公開されため、
識別子名の体裁(キャメルスタイル等)を決めること以外に説明することがほとんどなくなった。

筆者がよく使う「生成AIに適切な識別子を生成させるためのプロンプト」を紹介する。
...
この際、[クラスのコメント](comment.md#SS_6_2_1)、[関数のコメント](comment.md#SS_6_2_2)のフォーマットを生成AIに伝えれば、
より良いコードが作られる。問題点を修正し、それを生成AIにに張り付け、
単体テストをgtestで書くように依頼すれば、単体テスト付きのコードが入手できる。


### 5.2.1 ファイル、ディレクトリの命名 <a id="SS_5_2_1"></a>
ファイル名(や[ファイルペア](cpp_idioms.md#SS_17_9_1)名)、ディレクトリ名は、ソフトウェアを構成する[パッケージ](cpp_idioms.md#SS_17_9_2)やその構成物から、
強い影響を受けるべきである。

従って、ここで定める命名ルールに従っていないソースコードをベースとした開発、
保守を行っているプログラマが、そのソースコードをここでの名称ルールに従わせる場合、
まずは「[ソースコードファイルとディレクトリ](programming_convention.md#SS_2_7)」の順守から始めなければならない。
...
### 5.2.2 ファイル名 <a id="SS_5_2_2"></a>
* ファイル名は、以下のような形式の文字列にするか、それらを'\_'によって連結した文字列にする。
    * 正規表現[a-z][a-z0-9]+で表される文字列

* ファイル名は、下記のようにクラス名から生成する「[ファイルの使用方法](programming_convention.md#SS_2_2_1)」参照)。
...
### 5.2.6 ローカル変数名(自動変数名、仮引数名、関数内static変数名) <a id="SS_5_2_6"></a>
...
### 5.2.7 メンバ変数名 <a id="SS_5_2_7"></a>
* ローカル変数名と同じルールで生成した文字列に[Trailing Underscore(末尾アンダースコア)](cpp_idioms.md#SS_17_11_4)を適用する。
  (visual studioで開発を行うプロジェクトを除きメンバ変数をm'\_'で始めことはしない)。
* [POD](core_lang_spec.md#SS_15_4_5)のメンバ変数の名前は、ローカル変数名と同じルールで生成する(末尾に'\_'はつけない)。
...
### 5.2.9 Accessorメンバ関数名 <a id="SS_5_2_9"></a>
* 慣習に従うと機能が類推しやすいため、 
  一般的な[Accessor](cpp_idioms.md#SS_17_1_6)メンバ関数には、以下のような接頭語をつける。
...
### 5.2.10 関数名 <a id="SS_5_2_10"></a>
* 非メンバ関数の名前は、型名と同じルールで生成する。

### 5.2.11 名前空間名 <a id="SS_5_2_11"></a>
...
### 5.2.12 テンプレート名 <a id="SS_5_2_12"></a>
* ファイル外部から参照されるテンプレートの名前は、型名と同じルールで生成する。
* ファイル外部から参照されないテンプレート(「[名前空間](programming_convention.md#SS_2_8_2)」参照)の名前は、
  ローカル変数名と同じルールで生成する。

### 5.2.13 テンプレートの仮引数名 <a id="SS_5_2_13"></a>
* templateの仮引数名は、以下のような形式の文字列にするか、定数名と同じルールで生成する。
    * 正規表現[A-Z][0-9]?で表される文字列(慣習的にはTやその周辺の大文字SやU等)

### 5.2.14 マクロ名 <a id="SS_5_2_14"></a>
* 二重読み込みの防御用以外のマクロの名前は、定数名と同じルールで生成する。
* 二重読み込みの防御用のマクロの名前は、「[二重読み込みの防御](programming_convention.md#SS_2_7_3)」で定めたルールで生成する。

### 5.2.15 その他の命名則 <a id="SS_5_2_15"></a>
