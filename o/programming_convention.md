...
* 上記以外でunionのような機能が必要な場合、
  [std::variant](term_explanation.md#SS_19_9_8)(「[std::variantとジェネリックラムダ](template_meta_programming.md#SS_13_7_2_2)」参照)を使用する
...
* 通常の関数の定義に「[autoパラメータによる関数テンプレートの簡易定義](term_explanation.md#SS_19_11_13)」の使用を避ける。
  [autoパラメータによる関数テンプレートの簡易定義](term_explanation.md#SS_19_11_13)を使ったインライン関数は柔軟すぎる。
...
* copyコンストラクタ、copy代入演算子は[copyセマンティクス](term_explanation.md#SS_19_18_2)に従わせる。
* copyコンストラクタ、copy代入演算子の引数はconstリファレンスにする。
* [RVO(Return Value Optimization)](term_explanation.md#SS_19_19_13)により、
  copyコンストラクタの呼び出しは省略されることがあるため、
  copyコンストラクタ、copy代入演算子はコピー以外のことをしない。
* copy代入演算子は[lvalue修飾](term_explanation.md#SS_19_16_2)をする。
...
* moveコンストラクタ、move代入演算子は[moveセマンティクス](term_explanation.md#SS_19_18_3)に従わせる。
...
* [サイクロマティック複雑度](term_explanation.md#SS_19_21_3)は15以下が好ましい。
...
* 仮引数(「[実引数/仮引数](term_explanation.md#SS_19_19_3)」参照)の数は、4個程度を上限とする。
  引数が多くなりすぎる場合、その関数の引数用の構造体を定義し、それを使用して関数を呼び出す。
  この場合、[指示付き初期化](term_explanation.md#SS_19_8_4)を使用する。

* 仮引数を関数の戻り値として利用しない場合
  (且つ仮引数が関数テンプレートの[ユニバーサルリファレンス](term_explanation.md#SS_19_15_4)でない場合)、
...
* [ユニバーサルリファレンス](term_explanation.md#SS_19_15_4)を仮引数とする関数テンプレートでは、仮引数は非constにする。
...
* 避けがたい理由なしに以下のシンタックスを使用しない。
    * [戻り値型を後置する関数宣言](term_explanation.md#SS_19_12_5)
    * [関数の戻り値型auto](term_explanation.md#SS_19_12_6)
    * [後置戻り値型auto](term_explanation.md#SS_19_12_7)
...
* 関数が複数の値を返す場合、[std::optional](term_explanation.md#SS_19_9_7)、std::pair、std::tupple、
...
* 関数はそれが不可避でない限り、[no-fail保証](term_explanation.md#SS_19_17_1)をする。
  [no-fail保証](term_explanation.md#SS_19_17_1)関数は[noexcept](term_explanation.md#SS_19_17_4)を使用してそのことを明示する。
* throwをせざるを得ない場合、最低でも[基本的な安全性の保証](term_explanation.md#SS_19_17_3)をする。
* STLコンテナ(std::string, std::vector等)が発生させるエクセプションはtry-catchせず
  (catchしてデバッグ情報を保存するような場合を除く)、
  プログラムをクラッシュさせる。try-catchしてもできることはない。
* 特別な理由がない限り、コンストラクタ呼び出しは[noexcept](term_explanation.md#SS_19_17_4)と宣言する。
...
* エクセプションをthrowしないことが確定している関数は、[noexcept](term_explanation.md#SS_19_17_4)と宣言する。
  move代入演算子を[noexcept](term_explanation.md#SS_19_17_4)と宣言することは特に重要である。
...
* [exception-unfriendly](term_explanation.md#SS_19_17_5)な関数はエクセプションを発生させないようにする。
...
* noexceptと宣言された関数へのポインタへ、noexceptでない関数のポインタを代入しない
  (C++17では[ill-formed](term_explanation.md#SS_19_19_5)になる)。
...
* ifの条件式が、コンパイル時に定まるのであれば、[constexpr if文](term_explanation.md#SS_19_11_12)を使用する。
...
* 規格上で[演算子のオペランドの評価順位](term_explanation.md#SS_19_19_2)が決まっていても、一見優先順位が分かりづらい式では、
  順序を明示するために丸括弧を使う。
...
* [単純代入](term_explanation.md#SS_19_19_4)のみからなる文を除き、1つの文で複数の代入を行わない。
...
* &&や||の論理演算子の右オペランドで[副作用](term_explanation.md#SS_19_21_6)のある処理をしない。
...
* オブジェクトのダイナミックな生成には、特別な理由がない限りnewを使用せず、
  ...
  また、特別な理由でnewした場合、そのポインタは[スマートポインタ](term_explanation.md#SS_19_9_5)で管理する。
...
* 上記例を除き、sizeof演算子のオペランドは一見[副作用](term_explanation.md#SS_19_21_6)を持っているような式を含んではならない。
...
* [Run-time Type Information](term_explanation.md#SS_19_3_12)を使用したラインタイム時の型による場合分けは、
  ...
  (「[等価性のセマンティクス](term_explanation.md#SS_19_18_1)」参照)。
...
* [注意] ローカルオブジェクトに対して[RVO(Return Value Optimization)](term_explanation.md#SS_19_19_13)が有効であれば、
  そのオブジェクトを戻り値にしても良い。
* [注意] stdのコンテナは、RVOが有効でなくてもmoveが行われるため、関数の戻り値として使用しても良い。
* [注意] std::stringについては、RVOに加えて、
  [SSO(Small String Optimization)](term_explanation.md#SS_19_19_14)が使用されていることが多い。
...
* 関数の戻り値にローカルオブジェクトを使用する場合、
  [RVO(Return Value Optimization)](term_explanation.md#SS_19_19_13)の阻害になるため、そのオブジェクトをstd::moveしない。
...
* [g++](term_explanation.md#SS_19_20_1)/[clang++](term_explanation.md#SS_19_20_2)等の優れたコンパイラを適切なオプションで使用することで、
  非推奨の機能、関数、クラスの使用を防ぐ。
...
* assert()はコンパイルオプションにより無効化されることがあるため、
  assert()の引数に[副作用](term_explanation.md#SS_19_21_6)のある式を入れない。
