...
* 単なるデータホルダー(アプリケーションの設定データを保持するようなクラス等)や、
  ...
  [凝集性](cpp_idioms.md#SS_21_12_10)が高くなるように設計する。
...
* 非静的メンバの[ハンドル](cpp_idioms.md#SS_21_12_7)を返すメンバ関数を持つオブジェクトが
...
* オブジェクトaの所有権を持たないオブジェクトは、
  オブジェクトaの[ハンドル](cpp_idioms.md#SS_21_12_7)をメンバ変数で保持することを出来る限り避ける
...
* 関数、メンバ関数はなるべく[スレッドセーフ](cpp_idioms.md#SS_21_12_2)に実装する。
...
* &&や||の論理演算子の右オペランドで[副作用](cpp_idioms.md#SS_21_12_13)のある処理をしない。
...
* 上記例を除き、sizeof演算子のオペランドは一見[副作用](cpp_idioms.md#SS_21_12_13)を持っているような式を含んではならない。
...
```
    ...
    localtime(), ttyname(), 
    ctermid(), tmpnam() (引数がNULLのとき、非[リエントラント](cpp_idioms.md#SS_21_12_3)になる)
```
...
* signalの扱いは極めて難しく、安定動作をさせるのは困難である。
  「シグナルの[リエントラント](cpp_idioms.md#SS_21_12_3)問題を解決でき、使用できる関数に制限がない」という利点があるため、
...
* assert()はコンパイルオプションにより無効化されることがあるため、
  assert()の引数に[副作用](cpp_idioms.md#SS_21_12_13)のある式を入れない。
