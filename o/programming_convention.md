...
* [サイクロマティック複雑度](cpp_jargon.md#SS_20_1_3)は15以下が好ましい。
...
* &&や||の論理演算子の右オペランドで[副作用](cpp_jargon.md#SS_20_1_6)のある処理をしない。
...
* 上記例を除き、sizeof演算子のオペランドは一見[副作用](cpp_jargon.md#SS_20_1_6)を持っているような式を含んではならない。
...
* assert()はコンパイルオプションにより無効化されることがあるため、
  assert()の引数に[副作用](cpp_jargon.md#SS_20_1_6)のある式を入れない。
