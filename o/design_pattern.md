...
## lightweight Pimpl <a id="SS_9_3"></a>
[Pimpl](design_pattern.md#SS_9_2)の解説で示したように依存関係をシンプルに保つには極めて有効なパターンではあるが、
...
一方で、「[Accessor](design_pattern.md#SS_9_4)」で紹介したsetterは、クラスのimmutabilityを下げる。
...
## Clone(仮想コンストラクタ) <a id="SS_9_6"></a>
オブジェクトコピーによる[スライシング](cpp_idioms.md#SS_21_8_3)を回避するためのイデオムである。
...
## NVI(non virtual interface) <a id="SS_9_7"></a>
...
## DI(dependency injection) <a id="SS_9_9"></a>
メンバ関数内でクラスDependedのオブジェクトを直接、生成する
(もしくは[Singleton](design_pattern.md#SS_9_10)オブジェクトや静的オブジェクト(std::coutやstd::cin等)に直接アクセスする)
クラスNotDIがあるとする。
...
(「[Named Constructor](design_pattern.md#SS_9_15)」参照)と呼ばれる。

[演習-Singleton](exercise_q.md#SS_22_9_14)  

## State <a id="SS_9_11"></a>
...
この性質は、[Factory](design_pattern.md#SS_9_14)や[Named Constructor](design_pattern.md#SS_9_15)の実装には不可欠であるが、
...
DI(「[DI(dependency injection)](design_pattern.md#SS_9_9)」参照)と組み合わせて使われることが多い。

「[Templateメソッド](design_pattern.md#SS_9_13)」の例にFactoryを適用したソースコードを下記する。
...
## Named Constructor <a id="SS_9_15"></a>
Named Connstructorは、[Singleton](design_pattern.md#SS_9_10)のようなオブジェクトを複数、生成するためのパターンである。
...
次に示したのは「[Factory](design_pattern.md#SS_9_14)」の例にこのパターンを適応したコードである。
...
あるいは、[DI(dependency injection)](design_pattern.md#SS_9_9)と組み合わせて、下記のような改善も有用である。
...
これを改めるためには、「[CRTP(curiously recurring template pattern)](design_pattern.md#SS_9_19)」が必要になる。

次に示すソースコードはVisitorとは関係がないが、
FileEntityVisitorから派生するクラスを下記クラス図が示すように改善することで、
単体テストが容易になる例である(「[DI(dependency injection)](design_pattern.md#SS_9_9)」参照)。
...
## CRTP(curiously recurring template pattern) <a id="SS_9_19"></a>
CRTPとは、
...
このパターンを用いて、「[Visitor](design_pattern.md#SS_9_18)」のFileEntityの3つの派生クラスが持つコードクローン
...
GUIアプリケーションを[MVC](design_pattern.md#SS_9_21)で実装する場合のModelがSubjectであり、
ViewがObserverNである。
...
ViewはModelの[Observer](design_pattern.md#SS_9_20)であるため、ModelはViewへ依存しない。
