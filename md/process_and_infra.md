...
* 「[ソースコードファイルとディレクトリ](---)」で定めたルールに従っている。
...
```cpp
    // @@@ essential/h/scoped_guard.h #0:0 begin
    // @@@ essential/h/scoped_guard.h #0:1 begin
    // @@@ essential/h/scoped_guard.h #0:2 begin
```
...
* 各種メトリクスの計測([ソフトウェア一般|サイクロマティック複雑度](---)、[凝集性の欠如](---)等)
* リリースパッケージの作成
