...
### 依存関係の整理
「[ソースコードファイルとディレクトリ](---)」で述べたようにソースコードを[パッケージ](---)分割し、
...
* [割り込みベクタ](---)/[assert(false)](---)状態から実行できる必要があるため、
...
### メモリ保護
組み込みソフトは、[フリースタンディング環境](---)で動作させることが一般的である。
...
一時的に[メモリ保護](---)を無効化すればよい。
これを[モニター](---)から実行できるコマンドとして実装すると便利である。

### 割り込みベクタ
ほとんどの組み込みソフトではIRQを捕捉するための処理は実装されているが、
その一方で、[CPU例外](---)を捕捉する処理が実装されていないことが多い。
...
[フリースタンディング環境](---)ではこれが適切に動作しないことが多い。
