...
|機能                                                     |説明                                                                        |
|:--------------------------------------------------------|:---------------------------------------------------------------------------|
| 型推論([テンプレートと型推論|auto](---))                |変数や、式の型の推論                                                        |
| [decltype](---)                                         |式の型を取得                                                                |
| [構文と制御構造|範囲for文](---)                         |インデックス変数なしのfor文                                                 |
| [特殊メンバ関数|初期化子リストコンストラクタ](---)      |`vector<int>` v = {1, 2, 3}; など。                                         |
| [一様初期化](---)                                       |コンストラクタの呼び出しを、波カッコで行う。|vector<int> v{1, 2, 3};        |
| [expression|rvalue](---)/[moveセマンティクス](---)      |[C++慣用語句|オブジェクトの所有権](---)の移転、高速化。                     |
| [rvalueリファレンス](---)                               |テンポラリオブジェクトをバインドできる`T&&`形式のリファレンス               |
| [言語拡張機能|ラムダ式](---)                            |関数オブジェクトをその場に書く                                              |
| [noexcept](---)                                         |関数の例外指定、例外を投げる可能性のある式かbool値を返す演算子              |
| [constexpr](---)                                        |定数式やconstexpr関数の定義                                                 |
| nullptr                                                 |ヌルポインタを表すポインタ[C++コア言語仕様|リテラル](---)                   |
| インライン名前空間                                      |ネストした名前空間に透過的にアクセスする。                                  |
| [ユーザー定義リテラル](---)                             |ユーザにより定義されたリテラル。標準ライブラリでは\_sや\_svなど             |
...
|機能                                                               |説明                                                          |
|:------------------------------------------------------------------|:-------------------------------------------------------------|
|[オブジェクト生成と初期化|特殊メンバ関数](---)のdefault/delete 宣言|自動定義される特殊関数の制御                                  |
...
|変換演算子のオーバーロードの[explicit](---)宣言                    |「 [演算子オーバーロード](---)」参照                          |
|friend 宣言の対象を拡張                                            |テンプレートパラメータ、エイリアスfriend 宣言できるように     |
|メンバ関数の[rvalue修飾](---)/[lvalue修飾](---)                    |オブジェクトのと値カテゴリでの呼び出し可能であることの指定    |
|[列挙型とバイト表現|スコープドenum](---)                           |スコープを持ち、型付の強いenum                                |
...
| [C++コア言語仕様|リテラル|生文字列リテラル](---) |文字列リテラルにRプレフィックスを付けることで、エスケープシーケンスを無視する。  |
| char16_tとchar32_t                               |UTF-16とUTF-32の文字型                                                           |
| UTF-8文字列リテラル                              |char の文字列をUTF-8エンコーディングするプレフィックス                           |
| [属性構文](---)                                  |[[attr]]構文による属性指定。[[noreturn]]など。                                   |
| [alignas](---)                                   |アライメントを指定する。                                                         |
| [alignof](---)                                   |アライメントを取得する。                                                         |
...
|機能                |説明                                 |
|:-------------------|:------------------------------------|
|[std::move](---)    |[moveセマンティクス](---)の利用      |
|[std::forward](---) |[perfect forwarding](---)の実装、利用|

#### stdコンテナ
[標準ライブラリとプログラミングの概念|コンテナ](---)全般が[moveセマンティクス](---)に対応したため、
...
[expression|rvalue](---)オブジェクトも受け取れて、move挿入ができるようになった。
...
|:----------------------------:|:--------------------------------------------------------|
| [std::thread](---)           | スレッドの生成                                          |
| [std::atomic](---)           | 単純なオブジェクトのアトミック処理                      |
| [std::mutex](---)            | スレッド間の競合の回避                                  |
| [std::lock_guard](---)       | ミューテックスのロック/アンロックを管理するためクラス   |
| [std::unique_lock](---)      | ミューテックスのロック/アンロックを管理するためクラス   |
| [std::scoped_lock](---)      | ミューテックスのロック/アンロックを管理するためクラス   |
...
|機能                                                  |説明                                                                             |
|:-----------------------------------------------------|:--------------------------------------------------------------------------------|
|[2進数リテラル](---)                                  |2進数を表す0bもしくは0Bプレフィックスを付けた数値リテラルの記述を可能に          |
|[関数の戻り値型auto](---)                             |戻り値の型を後置する関羽鵜テンプレート/関数定義とラムダ式の戻り値型にautoに      |
|[decltype(auto)](---)                                 |decltypeの規則による変数の型推論                                                 |
|[言語拡張機能|ラムダ式](---)の初期化キャプチャ        |キャプチャに代入構文を導入し、一つの変数に複数のキャプチャ方法を指定可能にする   |
|[ジェネリックラムダ](---)                             |ラムダ式のパラメータを auto にすることで、ジェネリックな関数呼び出しに           |
|[テンプレートと型推論|変数テンプレート](---)          |変数定義時のテンプレート指定を可能に                                             |
|constexpr関数内での条件分岐とループの文を許可         |[constexpr if文](---)                                                            |
|structの[NSDMI](---)の対応                            |                                                                                 |
|structのネストする初期化における波カッコ省略を許可    |一様初期化と初期化子リストを組み合わせた際、二重に波カッコが不要に               |
|[[deprecated]]属性                                    |「[属性構文](---)」参照                                                          |
|[数値リテラル](---)の桁区切り文字                     |シングルクォーテーションで数値リテラルを桁区切りする                             |
|サイズ付きデアロケーション                            |[グローバルnew/deleteのオーバーロード](---)                                      |
...
|機能                                              |説明                                                      |
|:-------------------------------------------------|:---------------------------------------------------------|
|std::make_unique<>()                              |「[スマートポインタ|std::unique_ptr](---)」参照           |
...
|機能                                            |説明                                                                             |
|:-----------------------------------------------|:--------------------------------------------------------------------------------|
|[16進浮動小数点数リテラル](---)                 |16進数表記で浮動小数点数リテラルを記述できるようにする                           |
|インライン変数                                  |inline 指定をすることで翻訳単位を跨いでひとつのオブジェクトになる変数を定義する  |
|[構造化束縛](---)                               |                                                                                 |
|[CTAD(Class Template Argument Deduction)](---)  |コンストラクタの引数からテンプレート引数の型推論する機能                         |
|[単一要素の波カッコ初期化を非配列とする](---)   |リストによる直接初期化の型推論の規則が変更された                                 |
|[[maybe_unused]]属性                            |「[属性構文](---)」参照                                                          | 
|[[nodiscard]]属性                               |「[属性構文](---)」、[グローバルnew/deleteのオーバーロード](---)」参照           |
|[RVO(Return Value Optimization)](---)           |右辺値を変数の初期化のために使用する場合、コピーもムーブも省略することを保証     |
|[演算子のオペランドの評価順位](---)             |C++14まで未規定であった部分式の評価順序を規定                                    |
|enum class変数の初期値のルール変更              |「[スコープドenum](---)」、 「[underlying type](---)」参照                       |
|[newのアライメント指定](---)                    |                                                                                 |
|[初期化付きif/switch文](---)                    |if文とswitch文の条件式と初期化を分離する構文                                     |
|[[fallthrough]]属性                             |「[属性構文](---)」参照                                                          |
...
|[定数式とコンパイル時評価|constexprラムダ](---) |ラムダ式の関数オブジェクトのconstexpr指定を可能に                                |
|[std::optional](---)                            |関数の戻り値の無効表現を行うためのクラステンプレート                             |
|[std::variant](---)                             |型安全なunion機能を提供するクラステンプレート                                    |
...
```
```cpp
    // @@@ example/cpp_improve/cpp17_ut.cpp #2:1 begin -1
```
...
```
```cpp
    // @@@ example/cpp_improve/cpp17_ut.cpp #3:1 begin
```
...
|[explicit(COND)](---)                          |explicit(true/false) のように explicit にboolパラメータを指定できるようにする                       |
...
|特殊化のアクセスチェック                       |                                                                                                    |
|[[no_unique_address]]属性追加                  |空の型のオブジェクトをほかのオブジェクトと共有する最適化を許可。「[属性構文](---)」参照             |
...
|[autoパラメータによる関数テンプレートの簡易定義](---)  |[ジェネリックラムダ](---)と同様に関数パラメータをの型をautoにできる。           |
...
```
```cpp
    // @@@ example/cpp_improve/cpp20_ut.cpp #3:1 begin
```
...
```
```cpp
    // @@@ example/cpp_improve/cpp20_ut.cpp #4:1 begin -1
```
...
```
```cpp
    // @@@ example/cpp_improve/cpp20_ut.cpp #5:1 begin -1
```
