...
* インスタンス、インスタンスへのポインタ、インスタンスへのリファレンス等に対して、
    ...
    * 文字列リテラルのアドレスを非constポインタ型変数に代入しない(「[C++標準|リテラル](---)」参照)。
...
* 正規表現を表す文字列リテラルを使用する場合、文字列内のエスケープシーケンスをエスケープするのでなく、
  には[C++標準|リテラル|生文字列リテラル](---)を使用する。これにより正規表現の可読性が向上する。
...
* 宣言時にポインタ変数の初期値が決まらない場合、nullptrで初期化する(「[C++標準|リテラル](---)」参照)。
...
### オブジェクトの所有権
* オブジェクトaの所有権
  (「[C++セマンティクス|オブジェクトの所有権](---)」参照)を持つオブジェクトもしくは関数は、
  オブジェクトaの解放責務を持つ。
