...
#### 整数型
* [型システムと算術の基礎|整数型](---)には、整数の基本型(intやlong等)を直接使わずに、
...
#### 浮動小数点型
* 小数演算に安易に[型システムと算術の基礎|浮動小数点型](---)を使用せず、
...
* クラスのstatic constの整数定数の代わりにenumを使うことは、
  ...
  代わりにstatic constexprインスタンス(「[定数式とコンパイル時評価|constexpr関数](---)」参照)を使用する。
  こうすることで定数の型を明示できる。
...
* メンバ変数(static constやstatic constexprメンバは定数とする)を持たない構造体は、
  ...
  (「[型とインスタンス|enum](---)」や「[列挙型とバイト表現|enum](---)」参照)等に使用しても良い。
* コンストラクタ以外のメンバ関数を定義しない。
    * [ディープコピー](---)(「[コンストラクタ](---)」参照)が必要な型は、structでなくclassで表す。
    * デフォルトコンストラクタを除く[オブジェクト生成と初期化|特殊メンバ関数](---)に対して、
      = defaultの明示をしない。
...
* 配列の全要素にアクセスするような繰り返し処理には[構文と制御構造|範囲for文](---)を使用する。
...
* インスタンス、インスタンスへのポインタ、インスタンスへのリファレンス等に対して、
    * [constexpr定数](---)をパラメータにした場合、コンパイル時に定数として評価できる関数は、
      [定数式とコンパイル時評価|constexpr関数](---)として宣言する。
    * コンパイル時に必ず定数として評価したい[定数式とコンパイル時評価|constexpr関数](---)には、
      [定数式とコンパイル時評価|consteval](---)を使用する。
    * constexprにはできないが、const(「[constインスタンス](---)」にはできるインスタンスは、
      constとして定義する。関数の仮引数になっているリファレンスやポインタをconstにすることは特に重要である
    * 文字列リテラルのアドレスを非constポインタ型変数に代入しない(「[C++コア言語仕様|リテラル](---)」参照)。
...
* 正規表現を表す文字列リテラルを使用する場合、文字列内のエスケープシーケンスをエスケープするのでなく、
  には[C++コア言語仕様|リテラル|生文字列リテラル](---)を使用する。これにより正規表現の可読性が向上する。
...
* [型システムと算術の基礎|算術型](---)の宣言に[AAAスタイル](---)が使えない場合、
  「代入演算子を伴わない[オブジェクト生成と初期化|一様初期化](---)」を使用する。
...
* リファレンスやポインタの宣言に[AAAスタイル](---)が使えない場合、
  「代入演算子を伴わない[オブジェクト生成と初期化|一様初期化](---)」か「=による初期化」を使用する。
  「代入演算子を伴う一様初期化」、「()による初期化」を使用しない。
...
* 配列の宣言には、「代入演算子を伴わない[オブジェクト生成と初期化|一様初期化](---)」を使用する。
...
* 宣言時にポインタ変数の初期値が決まらない場合、nullptrで初期化する(「[C++コア言語仕様|リテラル](---)」参照)。
...
* 単なるデータホルダー(アプリケーションの設定データを保持するようなクラス等)や、
  ...
  [ソフトウェア一般|凝集性](---)が高くなるように設計する。
* [クラス凝集性のクライテリア](---)に従い、凝集性を判断し、凝集性が著しく低いクラスを作らないようにする。
...
* コンパイル時に戻り値が確定するメンバ関数は[定数式とコンパイル時評価|constexpr関数](---)と宣言する。
...
* クラスが解放責務を持つポインタ型メンバ変数を持つならば、copyコンストラクタ、
  ...
  (このルールはファイルディスクリプタ等のリソース管理をするクラス全般に当てはまる)。
    * [ディープコピー](---)をさせる。
    * = deleteする(「[オブジェクト生成と初期化|特殊メンバ関数](---)」参照)。
...
#### 初期化子リストコンストラクタ
* [特殊メンバ関数|初期化子リストコンストラクタ](---)は、コンテナクラスの初期化のためのみに定義する。
...
* クラスの可変な状態を表すメンバ変数(non-const, non-static)は4個以内に留める(「[クラス凝集性のクライテリア](---)」参考)。
...
#### オブジェクトのライフタイム
* [オブジェクト生成と初期化|オブジェクトのライフタイム](---)開始前、
...
### 実引数/仮引数
* 仮引数(「[言語仕様の定義要素|実引数/仮引数](---)」参照)の数は、4個程度を上限とする。
...
* 引数が[constexpr](---)の場合、コンパイル時に評価が確定する関数テンプレートもしくはinline関数は、
  [定数式とコンパイル時評価|constexpr関数](---)として宣言する。
* [定数式とコンパイル時評価|constexpr関数](---)がコンパイル時に評価される必要がある場合、
  [constexpr](---)の代わりに[定数式とコンパイル時評価|consteval](---)を使用する。
...
* コンパイル時に戻り値が確定する関数は[定数式とコンパイル時評価|constexpr関数](---)として宣言する。
...
* 配列やコンテナの全要素にアクセスするような繰り返し処理には、
  ...
  が避けられ、従来よりもシンプルに記述できる[構文と制御構造|範囲for文](---)を使用する。
...
* 独自のコンテナクラスを定義する場合、標準ライブラリのコンテナと同様の要件を満たすbegin()、end()や、
  cbegin()、cend()も定義し、そのコンテナに[構文と制御構造|範囲for文](---)を適用できるようにする。
...
* アセンブラ関数も、関数/メンバ関数のルールに従う(「[関数](---)」参照)。
...
* 浮動小数点型をなるべく使わない([型システムと算術の基礎|浮動小数点型](---))。
